...
ll cost=INT_MAX;
for(auto y:min_edges){
    ll curr_cost=0;
    vector<pair<pair<ll,ll>,ll>>curr_edges;
    //resetting DSU vectors
    parent.assign(n,-1);
    set_size.assign(n,0);
    for(ll i=0;i<n;i++)make(i);
    
    for(ll i=0;i<e;i++){
        //removing one edge present in final MST at a time
        if(i==y)continue;
        auto x=edges[i];
        ll weight=x.second;
        ll a=(x.first).first;
        ...
        if(find_par(a)!=find_par(b)){
            union_set(a,b);
            curr_edges.push_back(x);
            curr_cost+=x.second;
        }     
    }
    if(curr_cost<cost){
        cost=curr_cost;
        edges_final=curr_edges;
    } 
}
    //printing the edges along with their weights in th MST
    cout<<"2nd smallest MST is: "<<endl;
